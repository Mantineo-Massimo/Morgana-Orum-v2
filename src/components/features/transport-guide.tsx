...
import { Bus, MapPin, Clock, ExternalLink, Info } from "lucide-react"
...
export function TransportGuide() {
    return (
        <div className="space-y-12">
            {/* Info Card */}
            <div className="mb-12 bg-[#18182e] rounded-3xl p-8 md:p-12 text-white relative overflow-hidden shadow-2xl">
                <div className="relative z-10 flex flex-col md:flex-row items-center gap-8">
                    ...
                    <div className="size-48 md:size-64 bg-white/5 rounded-full flex items-center justify-center backdrop-blur-3xl border border-white/10 shrink-0">
                        <Bus className="size-24 text-white/20" />
                    </div>
                </div>
                {/* Pattern Decorativo */}
                <div className="absolute top-0 right-0 w-64 h-64 bg-[#f9a620]/20 blur-[120px] rounded-full -translate-y-1/2 translate-x-1/2 pointer-events-none"></div>
            </div>
                    ...
                    <div key={i} className="bg-white rounded-3xl border border-zinc-100 shadow-sm overflow-hidden flex flex-col h-full hover:shadow-xl transition-all group">
                        <div className="p-6 border-b border-zinc-50 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                <div className="size-10 rounded-xl bg-[#18182e] flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform">
                        ...
                        <div className="p-6 flex-1 space-y-3 bg-zinc-50/30">
                            {item.lines.map((line, li) => (
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex items-center gap-4 p-3 bg-white rounded-2xl border border-zinc-100 shadow-sm group/line hover:border-[#f9a620]/30 transition-all hover:shadow-md active:scale-[0.98]"
                                >
                                    <span className="shrink-0 size-10 flex items-center justify-center bg-[#18182e] text-white rounded-lg text-xs font-black px-2 shadow-md">
                                        {line.id}
                                    </span>
                                    <div className="flex-1">
                                        <p className="text-sm font-bold text-zinc-800 leading-tight group-hover/line:text-[#f9a620] transition-colors">
                                            {line.name}
                                        </p>
                                    </div>
                                    <ExternalLink className="size-3 text-zinc-300 group-hover/line:text-[#f9a620] transition-colors" />
                                </a>
                            ))}
                        </div>
                        <div className="p-6 pt-0 mt-auto bg-zinc-50/30">
                            <div className="p-4 bg-white/55 rounded-2xl border border-dashed border-zinc-200/80 flex items-center gap-3">
            ...
            {/* Shuttle Recall */}
            <div className="mt-20">
                <div className="bg-[#f9a620]/5 rounded-[40px] p-8 md:p-16 border border-[#f9a620]/10 flex flex-col md:flex-row items-center gap-12 text-center md:text-left">
                    <div className="flex-1">
                        <h2 className="text-3xl md:text-5xl font-serif font-black text-foreground mb-6 leading-tight uppercase tracking-tighter">
                            Il Cuore della Mobilità: <span className="text-[#f9a620]">Shuttle 100</span>
                        </h2>
                        ...
                        <div className="flex flex-wrap gap-4 justify-center md:justify-start">
                            ...
                            <div className="px-6 py-3 bg-white rounded-2xl shadow-sm border border-zinc-100 flex items-center gap-3">
                                <MapPin className="size-4 text-[#f9a620]" />
                    ...
                    <div className="shrink-0">
                        <div className="size-48 md:size-72 bg-white rounded-[50px] shadow-2xl flex items-center justify-center p-8 rotate-3 hover:rotate-0 transition-transform duration-500 border border-zinc-100">
                            <Bus className="size-32 text-[#f9a620]" />
                        </div>
                    </div>
                </div>
            </div>
