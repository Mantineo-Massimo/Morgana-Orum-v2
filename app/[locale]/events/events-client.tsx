...
function getBookingLabel(event: EventItem, t: any, locale: string): { label: string; color: string; icon: typeof Ticket } {
    if (!event.bookingOpen) {
        return { label: t("booking_closed"), color: "text-zinc-400 bg-zinc-100 border-zinc-200", icon: Lock }
    }

    const now = new Date()
    if (event.bookingStart && now < new Date(event.bookingStart)) {
        return { label: t("booking_opens", { date: formatShortDate(event.bookingStart, locale) }), color: "text-amber-600 bg-amber-50 border-amber-200", icon: Clock }
    }
    if (event.bookingEnd && now > new Date(event.bookingEnd)) {
        return { label: t("booking_expired"), color: "text-red-500 bg-red-50 border-red-200", icon: Lock }
    }
    return { label: t("booking_open"), color: "text-green-600 bg-green-50 border-green-200", icon: Ticket }
}
...
export default function EventsClient({
    ...
    const filteredEvents = events.filter(item => {
        ...
        if (activeCategory !== t("all_categories") && !itemCats.includes(activeCategory)) return false
...
const EventCard = forwardRef<HTMLDivElement, { item: EventItem }>(
    ({ item }, ref) => {
        const t = useTranslations("Events")
        const locale = useLocale()
        const booking = getBookingLabel(item, t, locale)
        ...
        const dateDisplay = hasEndDate
            ? `${formatShortDate(item.date, locale)} – ${formatShortDate(item.endDate!, locale)}`
            : dateStr
                ...
                >
                        ...
                        <div className="absolute top-4 left-4 z-20">
                            <div className="bg-white/95 backdrop-blur-sm text-center p-2 rounded-xl min-w-[3.5rem] shadow-lg border border-white/20">
                                ...
                                <span className="block text-2xl font-black leading-none text-foreground mt-0.5">
                                    {new Date(item.date).toLocaleDateString(locale, { day: '2-digit' })}
                                </span>
                            </div>
                        </div>
                        ...
                        {item.isRegistered && (
                            <div className="absolute bottom-3 right-3 z-20">
                                <span className="text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full bg-green-500/95 backdrop-blur-sm text-white shadow-sm flex items-center gap-1 border border-green-400">
                                    <CheckCircle className="size-3" /> {t("registered")}
                                </span>
                            </div>
                        )}
                    ...
                    <div className="p-6 flex flex-col flex-1">
                        ...
                        <div className="flex flex-col gap-1.5 mb-4 text-xs text-zinc-500 font-medium">
                            ...
                            <div className="flex items-center gap-2">
                                <Clock className="size-3.5 shrink-0" /> {formatTime(item.date, locale)}
                            </div>
                            <div className="flex items-center gap-2">
